...
This document describes the design of Custom ACL Base Metering (CABM) feature in SONiC.
...
- Support multiple rules within ACL to be bound to one policers.
- Query and validate SAI capabilities dynamically.
#### CLI Requirements
...
- An existing mechanism allows to define custom ACL table types and specify the desired combination of actions in the configuration (ACL User Defined Table Type HLD).
- To support the new policer action, the custom table type mechanism will be extended with the policer action attribute - SAI_ACL_ACTION_TYPE_SET_POLICER.
...
- - The existing design of SONiC ACL allows only one action to be defined per rule, this behavior will be kept.
...
-----------    ---------------    ---------  -------------------------- -------  -----------------
MY_ACL_1       Custom_1_POLICER   Ethernet2  Limit some traffic flows    Ingress  Inactive
                                  Ethernet4
                                  Ethernet7
